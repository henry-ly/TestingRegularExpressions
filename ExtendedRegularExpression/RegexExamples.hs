...
--import Text.Regex.Base           -- regex-pcre / regex-pcre-builtin
...
instance Arbitrary (R Char) where
     arbitrary = sized arbitraryExpression

arbitraryExpression 0 = frequency[(2, liftM Atom alphabet)]
arbitraryExpression n = frequency[(2, liftM2 (:+:) subexpr subexpr)
                                 ,(2, liftM2 (:>:) subexpr subexpr)
                                 ]
                   where subexpr = arbitraryExpression (n `div` 2)


prop_Alt :: R Char -> R Char -> String -> Bool
prop_Alt a b s = ((s =~ showExpr(a :+: b))::Bool) == ((s =~ (showExpr (b :+: a)))::Bool) 
