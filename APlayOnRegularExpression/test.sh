...
ghc aPlayOnRegularExpression.hs

echo "Testing aPlayOnRegularExpression.hs..."

./aPlayOnRegularExpression > result.txt
...
echo
echo "prop_Clo3"
echo "Succesful: $(awk '/prop_Clo3/,/END/' result.txt | grep -c "OK") Failed: $(awk '/prop_Clo3/,/END/' result.txt | grep -c "Failed")"
